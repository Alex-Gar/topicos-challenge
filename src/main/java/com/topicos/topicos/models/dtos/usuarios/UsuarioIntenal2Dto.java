...
public record UsuarioIntenal2Dto(
    ...
    public UsuarioIntenal2Dto(Usuario usuario) {
        this(
                ...
                usuario.getNombre(),
                usuario.getEmail(),
                usuario.getTopicos() != null ? usuario.getTopicos().stream().map(t -> new TopicoInternal2Dto(
                        t.getId(),
                        t.getTitulo(),
                        t.getMensaje(),
                        new CursoRequestDto(t.getCurso().getId(), t.getCurso().getNombre(), t.getCurso().getCategoria())
